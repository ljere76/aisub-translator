...
def translate_line_ollama(text, episode_synopsis, previous_context, original_language, target_language, model):
    """
    Traduit une ligne de texte en utilisant Ollama.
    """
    formatting_codes, plain_text = extract_leading_formatting(text)
    parts = split_text_with_formatting(plain_text)
    text_to_translate = " ".join(part for is_text, part in parts if is_text).strip()
    ...
    if previous_context:
        prompt += f"\nPrevious Dialogue Context (for context only): {previous_context}"
    prompt += f"\nNow, translate this: {text_to_translate}"

    logger.debug(f"Ollama request prompt:\n{prompt}")
    try:
        ...
        logger.error(f"Ollama API request failed for text: {text} with error: {e}")
        return None
...
def main():
    ...
    ollama_model = os.getenv("OLLAMA_MODEL")  # Nouveau : modèle Ollama à utiliser

    # Configuration des modèles depuis .env
    ...
    if ollama_model:  # Ajout de la clé Ollama
        available_keys["ollama"] = ollama_model

    if not available_keys:
        logger.error("No API keys or models found in .env file. Please set at least one of OPENAI_API_KEY, CLAUDE_API_KEY, DEEPSEEK_API_KEY, or OLLAMA_MODEL.")
        return

    parser = argparse.ArgumentParser(
        description="Translate subtitle files (.ass, .srt) line-by-line with context using an AI API. "
                    "Supports OpenAI, Claude, DeepSeek, and Ollama. For ASS files, the header is preserved."
    )
    ...
    parser.add_argument("--ai", choices=["openai", "claude", "deepseek", "ollama"], help="Which AI provider to use (openai, claude, deepseek, ollama)")
    ...
    if len(available_keys) == 1:
        ...
        logger.info(f"Only {ai_choice} API key/model found. Using {ai_choice}.")
    else:
        if not args.ai:
            logger.error("Multiple API keys/models found. Please specify which AI to use with the --ai argument (openai, claude, deepseek, ollama).")
            return
        ai_choice = args.ai
        if ai_choice not in available_keys:
            logger.error(f"API key/model for {ai_choice} is not available. Available keys/models: {', '.join(available_keys.keys())}")
            return
    ...
    elif ai_choice == "ollama":  # Nouveau : gestion de l'option Ollama
        translate_func = lambda text, ep_syn, prev_ctx, orig_lang, tgt_lang: translate_line_ollama(
            text, ep_syn, prev_ctx, orig_lang, tgt_lang, available_keys["ollama"]
        )
    else:
